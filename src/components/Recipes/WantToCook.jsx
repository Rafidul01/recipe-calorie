        ...
        <table className="table">
          ...
          <tbody className="text-[#282828B2] text-base font-normal">
            {/* row 1 */}
            {
              ...
              carts.map((newCart,inx) => <tr key={newCart} className="hover ">
              <th>{inx+1}</th>
              <td>{newCart.recipe_name}</td>
              <td>{newCart.preparing_time} minutes </td>
              <td>{newCart.calories} calories</td>
              <td><button className="btn bg-[#0BE58A] rounded-[24px] lg:font-medium font-thin w-[60px] lg:w-auto text-xs lg:text-base">Preparing</button></td>
            </tr> )
            }
          </tbody>
        </table>
